...
Shared OTA wire protocol: manifest schema, status/event enums, request/response contracts (Go + KMP-friendly).

## Boundary (decoupling)

Pure contracts and types only — no business logic, no transport, no I/O. Consumed by the server, the rollout engine, the validator, and every device agent.
